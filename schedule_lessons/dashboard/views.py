...
def relationships(request):
    ...
    if request.user.profile.user_type == 'tutor':
        ...
        no_results_found = request.GET.get('no_search_result')
        context['requested_tutors'] = requested_tutors
    ...
    if not request.user.is_anonymous:
        ...
        if rel_exists:
            ...
            if relationship.pending and relationship.created_by != request.user:
                ...
                Notification(user=student, message=message, created_on=datetime.datetime.now(tz=UTC_ZONE), picture=request.user.profile.profile_pic, link=url).save()
            else:
                return HttpResponse(status=400)
        else:
    ...
    if not request.user.is_anonymous:
        ...
        if rel_exists:
            ...
            if relationship.pending and relationship.created_by != request.user:
                ...
                message = "{} has accepted your friend request.".format(request.user.get_full_name())
                url = "/dashboard/profile/{}".format(request.user.profile.id)
                Notification(user=tutor, message=message, created_on=datetime.datetime.now(tz=UTC_ZONE), picture=request.user.profile.profile_pic, link=url).save()
            else:
                return HttpResponse(status=400)
        else:
...
def confirm_lesson(request, lesson_id):
    try:
        ...
        if lesson_to_confirm and (lesson_to_confirm.tutor == request.user or lesson_to_confirm.student == request.user) and request.user != lesson_to_confirm.created_by:
            ...
            Notification(user=person_to_schedule_with, message=message, created_on=datetime.datetime.now(tz=UTC_ZONE), picture=request.user.profile.profile_pic, link=url).save()
        return redirect('agenda')
    except Exception as e:
        print(str(e))
        return redirect('agenda')
